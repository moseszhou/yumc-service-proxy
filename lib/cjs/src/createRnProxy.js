...
function createRnProxy(originalService, serviceName, options = {}) {
    ...
    if (existingProxyRef) {
        console.warn(`[ServiceProxy] Service "${serviceName}" has already been proxied. ` + `Returning existing proxy instance to avoid duplicate creation.`);
    ...
    if (removeFromGlobal) {
        try {
            ...
            if (options.debug) {
                console.log(`[ServiceProxy:${serviceName}] Removed from global NativeModules for security. ` + `Access is now only available through the proxy.`);
            }
        }
        catch (error) {
    ...
    const inWhiteList = (property) => {
        return (typeof property === 'string' &&
            ...
            (property === 'canIUse' || config.properties.includes(property) || constant_1.OBJECT_PROPERTIES.includes(property)));
    };
    const canIUse = (0, canIUse_1.attachCanIUse)(mergedService, {
